    ...
    public partial class MainWindow : Window
    {
        ...
        private async Task DownloadHentaieraPageAsync(GalleryItem item, string galleryId, int pageNum, string targetPath, CancellationToken token)
        {
            ...
            if (!string.IsNullOrEmpty(imgUrl))
            {
                ...
                // If it is webp from hentaiera.com, convert it to jpg
                if (imgUrl.Contains("hentaiera.com") && imgUrl.EndsWith(".webp", StringComparison.OrdinalIgnoreCase))
                {
                    imgUrl = Regex.Replace(imgUrl, @"\.webp$", ".jpg", RegexOptions.IgnoreCase);
                }

                string actualExt = Path.GetExtension(imgUrl);
                
                // Ensure extension is strictly allowed
                string[] allowedExts = new string[] { ".jpg", ".png", ".jpeg", ".bmp", ".gif" };
