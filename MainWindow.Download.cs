...
namespace get_link_manga
{
    public partial class MainWindow : Window
    {
        ...
        private void BtnOpenFolder_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                Log("Äang táº£i xuá»‘ng, táº¡m khÃ³a má»Ÿ thÆ° má»¥c Ä‘á»ƒ trÃ¡nh má»Ÿ Explorer láº·p láº¡i.");
                return;
            }
            ...
            if (!Directory.Exists(path))
            {
                ...
                {
                    MessageBox.Show($"KhÃ´ng thá»ƒ táº¡o thÆ° má»¥c: {ex.Message}", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
            }
            ...
            {
                MessageBox.Show($"KhÃƒÂ´ng thÃ¡Â»Æ’ mÃ¡Â»Å¸ thÃ†Â° mÃ¡Â»Â¥c: {openError}", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private async void BtnStartDownload_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show("KhÃ´ng thá»ƒ download truyá»‡n Ä‘Æ°á»£c trÃªn nhentai.", "Cáº£nh bÃ¡o", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
        ...
        private void BtnPauseDownload_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (_isDownloadPaused)
            {
                ...
                btnPauseDownload.Content = "â¸ï¸";
                btnPauseDownload.Tag = "Pause";
                Log("ÄÃ£ tiáº¿p tá»¥c táº£i xuá»‘ng (Download resumed).");
            ...
            else
            {
                ...
                Log("ÄÃ£ táº¡m dá»«ng táº£i xuá»‘ng (Download paused).");
                foreach (var item in _scrapedItems)
                {
        ...
        private void BtnStopDownload_Click(object sender, RoutedEventArgs e)
        {
            if (_downloadCts != null)
            {
                _downloadCts.Cancel();
                _isDownloadPaused = false;
                btnStopDownload.Content = "â¹ï¸...";
                btnStopDownload.IsEnabled = false;
                btnPauseDownload.IsEnabled = false;
                Log("Äang dá»«ng quÃ¡ trÃ¬nh táº£i xuá»‘ng... (Stopping download process...)");
        ...
        internal async Task StartDownloadProcessAsync(System.Collections.Generic.List<GalleryItem> itemsToDownload)
        {
            if (_downloadCts != null)
            {
                MessageBox.Show("Má»™t tiáº¿n trÃ¬nh táº£i Ä‘ang cháº¡y. Vui lÃ²ng dá»«ng láº¡i trÆ°á»›c khi báº¯t Ä‘áº§u táº£i má»›i (A download process is already running).", "Information", MessageBoxButton.OK, MessageBoxImage.Information);
                return;
            }
            ...
            if (!Directory.Exists(downloadRoot))
            {
                ...
                {
                    MessageBox.Show($"KhÃ´ng thá»ƒ táº¡o thÆ° má»¥c lÆ°u: {ex.Message}", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
            }
            ...
            Log($"Báº¯t Ä‘áº§u táº£i song song {totalGalleries} truyá»‡n vá»›i tá»‘i Ä‘a {maxParallelBooks} truyá»‡n cÃ¹ng lÃºc...");
            ...
            try
            {
                        ...
                        tasks.Add(Task.Run(async () =>
                        {
                            try
                            {
                                ...
                                foreach (var item in bookGroup)
                                {
                                    ...
                                    Log($"[Download] Äang táº£i: {item.Name} ({item.Link})");
                                    ...
                                    try
                                    {
                                        ...
                                        Log($"[Download] HoÃ n thÃ nh truyá»‡n: {item.Name}");
                                    ...
                                    {
                                        Log($"[Lá»—i] KhÃ´ng thá»ƒ táº£i truyá»‡n '{item.Name}': {ex.Message}");
                                        Dispatcher.Invoke(() =>
                                        {
                                            item.Status = "Error";
                ...
                lblStatus.Text = "Táº£i xuá»‘ng hoÃ n táº¥t! (Downloads completed)";
                Log("Táº£i xuá»‘ng toÃ n bá»™ thÃ nh cÃ´ng!");
                MessageBox.Show("ÄÃ£ táº£i xong toÃ n bá»™ truyá»‡n Ä‘Æ°á»£c chá»n!", "Success", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (OperationCanceledException)
            {
                Log("QuÃ¡ trÃ¬nh táº£i xuá»‘ng Ä‘Ã£ bá»‹ dá»«ng bá»Ÿi ngÆ°á»i dÃ¹ng.");
        ...
        private async Task DownloadGalleryAsync(GalleryItem item, string rootFolder, CancellationToken token, GalleryItem queueItem = null, ChapterFilter chapterFilter = null)
        {
            ...
            {
                Log($"[Bá» qua] nhentai chá»‰ dÃ¹ng Ä‘á»ƒ get link, khÃ´ng há»— trá»£ táº£i: {item.Name}");
                return;
            }
            ...
            Log($"[Äa luá»“ng] Báº¯t Ä‘áº§u táº£i {totalPages} trang vá»›i tá»‘i Ä‘a {maxThreads} káº¿t ná»‘i song song...");
            ...
            using (var semaphore = new DynamicSemaphore(maxThreads, GetCurrentConnectionLimit))
            {
                    ...
                    tasks.Add(Task.Run(async () =>
                    {
                        ...
                        try
                        {
                            ...
                            if (isFastPath)
                            {
                                ...
                                {
                                    Log($"[Fast Path] Lá»—i trang {pageNum} ({ex.Message}). Thá»­ Slow Path fallback...");
                ...
                {
                    Log($"[Lá»—i] KhÃ´ng thá»ƒ di chuyá»ƒn thÆ° má»¥c táº¡m HentaiForce: {ex.Message}");
                }
                finally
                {
        ...
        private async Task DownloadPageSlowPathAsync(GalleryItem item, int pageNum, string targetPath, CancellationToken token)
        {
            ...
            {
throw new Exception($"KhÃ´ng thá»ƒ trÃ­ch xuáº¥t Ä‘á»‹a chá»‰ áº£nh tá»« trang Ä‘á»c {pageNum}");
            }
        }
        ...
        private async Task DownloadNhentaiGalleryAsync(GalleryItem item, string rootFolder, CancellationToken token)
        {
            ...
            if (isCdnLink)
            {
                ...
                if (totalPages == 0)
                {
                    throw new Exception($"KhÃ´ng thá»ƒ xÃ¡c Ä‘á»‹nh sá»‘ lÆ°á»£ng trang hoáº·c trang khÃ´ng kháº£ dá»¥ng cho CDN Gallery: {mediaId}");
                }
            }
            else
            {
                ...
                {
                    // Likely Cloudflare 403/503 â€” ask user to solve captcha once then retry
                    bool ok = await SolveNhentaiCaptchaIfNeededAsync(item.Link);
                    if (!ok)
                        throw new Exception("KhÃ´ng thá»ƒ vÆ°á»£t qua Cloudflare. Táº£i xuá»‘ng bá»‹ há»§y.");
                    html = await _httpClient.GetStringAsync(item.Link);
                }
                ...
                if (patternMatch.Success)
                {
                    ...
                    Log($"[nhentai] PhÃ¡t hiá»‡n CDN prefix: {prefix}, ext: {ext}, total pages: {totalPages}");
                }
                else
                {
                    Log($"[nhentai] Cáº£nh bÃ¡o: KhÃ´ng tÃ¬m tháº¥y thumbnail pattern trong trang gallery. Sáº½ thá»­ slow path.");
                }
            }
            ...
            Log($"[Äa luá»“ng nhentai] Báº¯t Ä‘áº§u táº£i {totalPages} trang, mode: {nhentaiModeLabel}, tá»‘i Ä‘a {maxThreads} káº¿t ná»‘i song song...");
            ...
            using (var semaphore = new DynamicSemaphore(maxThreads, GetCurrentConnectionLimit))
            {
                        ...
                        try
                        {
                            ...
                            if (alreadyExists)
                            {
                                lock (lockObj)
                                {
                                    ...
                                    Dispatcher.Invoke(() =>
                                    {
                                        lblStatus.Text = $"[{completedPages}/{totalPages}] Táº£i {safeTitle} ({modeText})";
                                    });
                                }
                                return;
                            }
                            ...
                            if (isFastPath)
                            {
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    if (!success)
                                    {
                                        Log($"[nhentai Fast Path] Lá»—i trang {pageNum} ({ex.Message}). Thá»­ Slow Path fallback...");
                ...
                {
                    Log($"[Lá»—i] KhÃ´ng thá»ƒ di chuyá»ƒn thÆ° má»¥c táº¡m nhentai: {ex.Message}");
                }
                finally
                {
                    UnregisterTempFolder(tempFolder);
        ...
        {
            Log($"[nhentai] Äang dÃ² tÃ¬m tá»•ng sá»‘ trang cho media ID {mediaId}...");
            
            // Check if page 1 exists
            string p1Ext = await FindValidExtensionAsync(mediaId, 1, defaultExt);
            if (p1Ext == null)
            {
                Log($"[nhentai] Lá»—i: KhÃ´ng thá»ƒ tÃ¬m tháº¥y trang 1 cho media ID {mediaId}");
                return 0;
            }
            ...
            Log($"[nhentai] ÄÃ£ dÃ² tÃ¬m xong. Tá»•ng sá»‘ trang phÃ¡t hiá»‡n: {detectedPages}");
        ...
        private async Task DownloadNhentaiPageSlowPathAsync(string galleryUrl, int pageNum, string targetPath, CancellationToken token)
        {
            ...
            {
                throw new Exception($"KhÃ´ng thá»ƒ trÃ­ch xuáº¥t Ä‘á»‹a chá»‰ áº£nh tá»« trang Ä‘á»c nhentai {pageNum}");
            }
        }
        ...
        internal async Task<bool> SolveNhentaiCaptchaIfNeededAsync(string testUrl)
        {
            ...
            try
            {
                ...
                _isDownloadPaused = true;
                Log("[nhentai.net] PhÃ¡t hiá»‡n thá»­ thÃ¡ch Cloudflare / Captcha. Táº¡m dá»«ng táº£i vÃ  Ä‘ang má»Ÿ trÃ¬nh duyá»‡t giáº£i tá»± Ä‘á»™ng...");
                ...
                if (solved)
                {
                    Log("[nhentai.net] Giáº£i captcha thÃ nh cÃ´ng. Tiáº¿p tá»¥c táº£i...");
                    _isDownloadPaused = false;
                    return true;
                }
                return false;
        ...
        internal async Task<bool> SolveViHentaiCaptchaIfNeededAsync(string testUrl)
        {
            ...
            try
            {
                ...
                _isCaptchaWindowActive = true;
                _isDownloadPaused = true;
                Log("[vi-hentai.pro] PhÃ¡t hiá»‡n thá»­ thÃ¡ch Cloudflare / Captcha. Táº¡m dá»«ng táº£i vÃ  Ä‘ang má»Ÿ trÃ¬nh duyá»‡t giáº£i tá»± Ä‘á»™ng...");
                ...
                if (solved)
                {
                    Log("[vi-hentai.pro] Giáº£i captcha thÃ nh cÃ´ng. Tiáº¿p tá»¥c táº£i...");
                    _isDownloadPaused = false;
                    return true;
                }
                return false;
        ...
        private async Task DownloadUrlToFileWithRefererAsync(string url, string referer, string filePath, CancellationToken token, bool isViHentai = false, bool isTruyenqq = false)
        {
            ...
            for (int attempt = 1; attempt <= maxAttempts; attempt++)
            {
                    ...
                    using (var request = new HttpRequestMessage(HttpMethod.Get, url))
                    {
                        ...
                        using (var response = await _httpClient.SendAsync(request, HttpCompletionOption.ResponseHeadersRead, token))
                        {
                            if (isViHentai && (int)response.StatusCode == 429 && attempt < maxAttempts)
                            {
                                ...
                                Log($"[vi-hentai.pro] 429 khi táº£i áº£nh. Chá» {retryDelay}ms rá»“i thá»­ láº¡i ({attempt}/{maxAttempts}): {url}");
                ...
                catch (HttpRequestException ex) when (attempt < maxAttempts)
                {
                    ...
                    Log($"{label} Thá»­ táº£i láº¡i áº£nh do lá»—i máº¡ng: {ex.Message}. Chá» {delayMs}ms ({attempt}/{maxAttempts}).");
                ...
                {
                    string label = isViHentai ? "[vi-hentai.pro]" : (isTruyenqq ? "[truyenqq]" : "[network]");
                    Log($"{label} Thá»­ táº£i láº¡i áº£nh do timeout. Chá» {delayMs}ms ({attempt}/{maxAttempts}).");
                    await Task.Delay(delayMs, token);
                    delayMs = Math.Min(delayMs * 2, 8000);
                }
            }

            throw new Exception($"KhÃ´ng thá»ƒ táº£i áº£nh sau {maxAttempts} láº§n thá»­: {url}");
        }
        ...
        private void ValidateDownloadedFiles(string folderPath, int expectedCount, GalleryItem queueItem, string chapterName = "General")
        {
            ...
            try
            {
                ...
                {
                    string missingMsg = $"Thiáº¿u cÃ¡c trang: {string.Join(", ", missingPages.Select(p => p.ToString("D3")))}";
                    Log($"[Cáº£nh bÃ¡o] ThÆ° má»¥c '{Path.GetFileName(folderPath)}' bá»‹ thiáº¿u {missingPages.Count} trang!");
                    if (queueItem != null)
                    {
                        Dispatcher.Invoke(() =>
                        {
                            ...
                            {
                                queueItem.AddError(chapterName, p, "Trang bá»‹ thiáº¿u (Missing page)", null);
                            }
                        });
                    }
                }
            ...
            {
                Log($"[Lá»—i] KhÃ´ng thá»ƒ kiá»ƒm tra tÃ­nh toÃ n váº¹n cá»§a thÆ° má»¥c '{folderPath}': {ex.Message}");
            }
        }
        ...
        private void CmbMultiDownload_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
        {
            ...
            Log($"[Multi Download] Sá»‘ luá»“ng táº£i song song Ä‘Æ°á»£c chá»‰nh thÃ nh {newVal}.");
    ...
    public partial class MainWindow : Window
    {
        private async Task DownloadHentaieraGalleryAsync(GalleryItem item, string rootFolder, CancellationToken token, GalleryItem queueItem = null, ChapterFilter chapterFilter = null)
        {
            ...
            try
            {
                ...
                catch (HttpRequestException)
                {
                    ...
                    if (!ok)
                        throw new Exception("KhÃ´ng thá»ƒ vÆ°á»£t qua Cloudflare cá»§a hentaiera.com. Táº£i xuá»‘ng bá»‹ há»§y.");
                    html = await _httpClient.GetStringAsync(item.Link);
                }
                ...
                Log($"[Hentaiera] Báº¯t Ä‘áº§u táº£i {totalPages} trang vá»›i tá»‘i Ä‘a {maxThreads} káº¿t ná»‘i song song...");
                ...
                using (var semaphore = new DynamicSemaphore(maxThreads, GetCurrentConnectionLimit))
                {
                    ...
                    {
                        Log($"[Lá»—i] KhÃ´ng thá»ƒ di chuyá»ƒn thÆ° má»¥c táº¡m Hentaiera: {ex.Message}");
                    }
                    finally
                    {
                        UnregisterTempFolder(tempFolder);
        ...
        private async Task DownloadHentaieraPageAsync(GalleryItem item, string galleryId, int pageNum, string targetPath, CancellationToken token)
        {
            ...
            catch (HttpRequestException)
            {
                ...
                if (!ok)
                    throw new Exception("Bá»‹ cháº·n bá»Ÿi Captcha khi táº£i trang xem.");
                viewHtml = await _httpClient.GetStringAsync(viewUrl);
            }
            ...
            {
                throw new Exception($"KhÃ´ng thá»ƒ trÃ­ch xuáº¥t Ä‘á»‹a chá»‰ áº£nh tá»« trang Ä‘á»c Hentaiera {pageNum}");
            }
        }
    }
}
